...
type (
	...
	OperationLog struct {
		...
		Target       sql.NullString `db:"target"`        // 操作对象描述
		Detail       sql.NullString `db:"detail"`        // 详细数据
		Ip           sql.NullString `db:"ip"`            // 操作IP
...
func (m *defaultOperationLogModel) Insert(ctx context.Context, data *OperationLog) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.OperatorId, data.OperatorName, data.Module, data.Action, data.Target, data.Detail, data.Ip)
	}, operationLogIdKey)
	return ret, err
}
...
func (m *defaultOperationLogModel) Update(ctx context.Context, data *OperationLog) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.OperatorId, data.OperatorName, data.Module, data.Action, data.Target, data.Detail, data.Ip, data.Id)
	}, operationLogIdKey)
	return err
}
