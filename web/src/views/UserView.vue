...
async function fetchList() {
  loading.value = true
  try {
    const res: any = await request.get('/api/user/list', {
...
function onAdd() {
  isEdit.value = false
  form.value = { name: '', phone: '', email: '', idCard: '', education: '', role: 0, status: 1, password: '' }
...
async function onDelete(row: User) {
  try {
    ...
    await request.post('/api/user/delete', { id: row.id })
...
<template>
  <div>
    <div class="toolbar">
      <el-input v-model="keyword" placeholder="搜索姓名/手机号" style="width: 240px; margin-right: 12px" @keyup.enter="onSearch" />
    ...
    <el-table :data="list" v-loading="loading" stripe>
      ...
      <el-table-column prop="role" label="角色" width="100">
        <template #default="{ row }">
          <el-tag>{{ row.role === 1 ? '管理员' : '普通用户' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="学历">
          <el-select v-model="form.education" style="width: 100%">
            ...
            <el-option label="在职研究生" value="在职研究生" />
